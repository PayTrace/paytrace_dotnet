...
namespace PayTrace.Integration
{
    public class CustomerRequest
    {
        ...
        public Response UpdateCustomer(string customerID)
        {
            var request = BuildCustomerRequest();
            request[Keys.CUSTID] = customerID;
            request[Keys.METHOD] = Methods.UpdateCustomer;
            return request.Send();
 
        }

        public void UpdateCustomerChangePassword(string customerID, string newpassword )
        {
            var request = new Request(Destination);
                   
            request = AddAuthorization(request);
            request[Keys.CUSTID] = customerID;
            request[Keys.CUSTPSWD] = newpassword;
            request[Keys.METHOD] = Methods.UpdateCustomer;
        }
